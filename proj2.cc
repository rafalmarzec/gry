...
void installTrafficGenerator(Ptr<ns3::Node> fromNode, Ptr<ns3::Node> toNode, int port, int packetSize, std::string tcpVariant, int simulationTime, int warmupTime);
...
int main (int argc, char *argv[])
{
	...
	string mcs;
    //std::string offeredLoad = "1"; //Mbps dla TCP bez sensu
	...
	for(int i = 0; i < APs; ++i){
		for(int j = 0; j < stations; ++j)
			...
			installTrafficGenerator(wifiStaNodes[i].Get(j),wifiApNodes.Get(i), port++, packetSize, tcpVariant, simulationTime, warmupTime);
	}
	...
	if (fileExists(outputCsv))
	{
		...
		myfile << "Timestamp,TcpVariant,RngRun,FlowSrc,FlowDst,Throughput,Delay" << std::endl;
	}
	...
	for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i) {
		...
		myfile << std::put_time(&tm, "%Y-%m-%d %H:%M") << "," << tcpVariant << "," << RngSeedManager::GetRun() << "," << t.sourceAddress << "," << t.destinationAddress << "," << flowThr << "," << flowDel;
		myfile << std::endl;
		totalThr += flowThr;
	}
	myfile.close();
