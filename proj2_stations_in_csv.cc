...
NS_LOG_COMPONENT_DEFINE ("ms-proj2");
...
void installTrafficGenerator(Ptr<ns3::Node> fromNode, Ptr<ns3::Node> toNode, int port, std::string offeredLoad, int packetSize, std::string tcpVariant, int simulationTime, int warmupTime);
...
int main (int argc, char *argv[])
{
	...
	string mcs;
    std::string offeredLoad = "10"; //Mbps
	int simulationTime = 10;
	int warmupTime = 1;
	int packetSize = 1472;
	string tcpVariant = "NewReno"; // default NewReno
	uint32_t dataRate = 300; // Aggregate traffic generator data rate [Mb/s]
	std::string outputCsv = "ms-proj2.csv";
	...
	cmd.AddValue ("tcpVariant", "Choose TCP variant", tcpVariant);
	...
	/* Set TCP Variant */
	Config::SetDefault ("ns3::TcpL4Protocol::SocketType", StringValue ("ns3::Tcp" + tcpVariant));
	...
	for(int i = 0; i < APs; ++i){
		for(int j = 0; j < stations; ++j)
			...
			installTrafficGenerator(wifiStaNodes[i].Get(j),wifiApNodes.Get(i), port++, offeredLoad, packetSize, tcpVariant, simulationTime, warmupTime);
	}
	...
	if (fileExists(outputCsv))
	{
		...
		myfile << "Timestamp,stations,OfferedLoad,TcpVariant,RngRun,FlowSrc,FlowDst,Throughput,Delay" << std::endl;
	}
	...
	for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i) {
		...
		myfile << std::put_time(&tm, "%Y-%m-%d %H:%M") << "," << stations << "," << offeredLoad << "," << tcpVariant << "," << RngSeedManager::GetRun() << "," << t.sourceAddress << "," << t.destinationAddress << "," << flowThr << "," << flowDel;
		myfile << std::endl;
		totalThr += flowThr;
	}
	myfile.close();
...
void installTrafficGenerator(Ptr<ns3::Node> fromNode, Ptr<ns3::Node> toNode, int port, std::string offeredLoad, int packetSize, std::string tcpVariant, int simulationTime, int warmupTime ) {
	...
	//OnOffHelper onOffHlper ("ns3::UdpSocketFactory", sinkSocket);
	OnOffHelper onOffHelper ("ns3::TcpSocketFactory", sinkSocket);

    //onOffHelper.SetConstantRate(DataRate(dataRate * 1e6 / nWifi), 1000);  //from jows -> Set data rate (150 Mb/s divided by no. of transmitting stations) and packet size [B]
